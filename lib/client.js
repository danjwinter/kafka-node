...
Client.prototype.danSendOffsetFetchV1Request = function (group, payloads, cb) {
  const coordinatorId = this.sendGroupCoordinatorRequest(group, (err, coordinatorInfo ) => {
    this.coordinatorId = String(coordinatorInfo.coordinatorId)
    var encoder;
    try {
      encoder = protocol.encodeOffsetFetchV1Request
      // encoder = protocol.danEncodeOffsetFetchRequest;
    } catch (e) {
      console.log('could not make encoder ', JSON.stringify(e))
    }
    console.log('ENCODER: ', JSON.stringify(encoder, null, 2))
    var decoder = protocol.decodeOffsetFetchV1Response;
    this.sendGroupRequest(encoder, decoder, arguments);
  })
};
...
Client.prototype.send = function (payloads, encoder, decoder, cb) {
  ...
  console.log("PAYLOADS IN SEND: ", JSON.stringify(payloads))
