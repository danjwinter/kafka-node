...
function decodeOffsetFetchV1Response (resp) {
  console.log(`Inside response: ${JSON.stringify(resp)}`)
  try {
  ...
  function decodePartitions (end, vars) {
    console.log(`On partition num: ${vars.partitionNum}`)
    if (--vars.partitionNum === 0) end();
    topics[vars.topic] = topics[vars.topic] || {};
    this.word32bs('partition')
      .word64bs('offset')
  ...
  console.log(`Post decode, topics: ${JSON.stringify(topics)}`)
  return topics;
  } catch (e) {
    console.log(`Threw an error in binary parse loop decode: ${e.message}`)
  }
}
