...
function groupByTopicToPartitions (payloads) {
  return payloads.reduce(function (out, p) {
    out[p.topic] = out[p.topic] || {};
    out[p.topic][p.partition] = p;
    return out;
...
function danEncodeOffsetFetchRequest (group) {
  return function (clientId, correlationId, payloads) {
    return _danEncodeOffsetFetchRequest(clientId, correlationId, group, payloads);
  };
}

function encodeOffsetFetchV1Request (clientId, correlationId, group, payloads) {
  // payloads = groupByTopic(payloads); // dan added
  console.log('PAYLOADS IN encode: ', JSON.stringify(payloads))
...
function _danEncodeOffsetFetchRequest (clientId, correlationId, group, payloads) {
  console.log(`payloads pre group: ${JSON.stringify(payloads, null, 2)}`)
  console.log(`clientId pre group: ${JSON.stringify(clientId, null, 2)}`)
  console.log(`correlationId pre group: ${JSON.stringify(correlationId, null, 2)}`)
  console.log(`group pre group: ${JSON.stringify(group, null, 2)}`)
  payloads = groupByTopic(payloads);
  var request = encodeRequestHeader(clientId, correlationId, REQUEST_TYPE.offsetFetch, 1);
  var topics = Object.keys(payloads);
  ...
  console.log(`about to try to encode request: ${JSON.stringify(request)}`)
  try {

  temp = encodeRequestWithLength(request.make());
  console.log(`encoded request: ${JSON.stringify(temp)}`)
  return temp
  } catch (e) {
    console.log(`Errored trying to encode request: ${e.message}`)
  }
...
exports.danEncodeOffsetFetchRequest = danEncodeOffsetFetchRequest;
