...
Client.prototype.danSendOffsetFetchV1Request = function (group, payloads, cb) {
  console.log(`BEFORE GROUP COORD REQUEST ARGS: ${arguments}`)

  console.log(`BEFORE GROUP COORD REQUEST payloads: ${JSON.stringify(payloads)}`)
  this.sendGroupCoordinatorRequest(group, (err, coordinatorInfo ) => {
    console.log(`GroupCoordinator response non-stringified: ${coordinatorInfo.coordinatorId}`)
    console.log(`sendGroupRequest argumets: ${arguments}`)
    this.coordinatorId = String(coordinatorInfo.coordinatorId)
    var encoder = protocol.danEncodeOffsetFetchRequest(group);
    var decoder = protocol.decodeOffsetFetchV1Response;
    this.sendGroupRequest(encoder, decoder, arguments);
  })
};
...
Client.prototype.sendGroupRequest = function (encode, decode, requestArgs) {
  console.log(`request args pre values assignment: ${JSON.stringify(requestArgs)}`)

  requestArgs = _.values(requestArgs);
  console.log(`request args pre values pop: ${JSON.stringify(requestArgs)}`)
  var cb = requestArgs.pop();
  var correlationId = this.nextId();
  console.log(`request args pre unshift with clientId ${this.clientId} and correlationId ${correlationId}: ${JSON.stringify(requestArgs)}`)
  requestArgs.unshift(this.clientId, correlationId);
  console.log(`requestArgs post unshift: ${JSON.stringify(requestArgs)}`)
  var request = encode.apply(null, requestArgs);
  console.log(`Request after encode: ${JSON.stringify(request, null, 2)}`)
